...
const formattedDate = (result: string) => {
  return moment(result).utc().format("YYYY-MM");
};

async function sendImageGemini(path: string) {
...
routes.patch("/confirm", async (req, res) => {
  ...
  if (typeof measure_uuid !== "string" || typeof confirmed_value !== "string") {
    return res.status(400).json({
      ...
      error_description:
        "Tipos de dados inválidos para measure_uuid ou confirmed_value.",
    });
  }
...
routes.get("/:customerCode/list", async (req, res) => {
  ...
  try {
    ...
    // prettier-ignore
    var query: FilterQuery<ImageFilter>  = { customer_code: customerCode.toUpperCase() };
